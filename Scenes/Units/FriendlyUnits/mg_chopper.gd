...
@export var mg_chopper_speed = 20
@export var mg_chopper_range = 100
...
func handle_target(delta): 
	...
	if in_attack_distance and (target.is_in_group("EnemyAirUnit") or target.is_in_group("EnemyGroundUnit")): 
		attack(delta)
		return true
...
func set_target(unit): 
	if unit == null: return 
	if not GlobalFunctions.is_in_groups(unit, ["EnemyAirUnit", "EnemyGroundUnit"]): return 
	target = unit
	target.select() 
